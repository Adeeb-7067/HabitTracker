...
// Habit tracking data structure with mock historical data
const habitData = [
  {
    id: 1,
    name: "Drink Water",
...
const HabitDropdownAndGraph = () => {
  ...
  // Process data when selectedHabits change
  useEffect(() => {
    // Create combined data for multi-line chart
    const days = ["Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun"];
    const combined = days.map(day => {
      // Create dataPoint with explicit type to fix the TypeScript error
      const dataPoint: { [key: string]: string | number } = { day };
      
      // Add progress for each selected habit
...
const StreakTracker = () => {
  ...
  // Days completed data - last 14 days (true for completed)
  const daysCompleted = [
  ...
  return (
    <div className="bg-black text-green-400 rounded-lg overflow-hidden w-full mx-auto shadow-lg border border-green-700">
      {/* Header */}
                    ...
                    return (
                      <div
                        ...
                        className={`flex flex-col items-center justify-center w-12 h-12 rounded-lg ${
                          isToday ? 'bg-green-900 border border-green-600 shadow-lg' : 
